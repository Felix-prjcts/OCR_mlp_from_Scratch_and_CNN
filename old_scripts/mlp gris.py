        ...
        for fichier in os.listdir(chemin_sous_dossier):
            ...
            try:
                with Image.open(chemin_image) as img:
                    pixels = np.array([sum(pixel) // 3 for pixel in img.getdata()], dtype=np.float32)/255
                    pixels = pixels.reshape(-1, 1)
                    dataset_gris_train.append((int(label), pixels))
            except Exception as e:
                print(f" Erreur avec {chemin_image}: {e}")
print("Valeurs min/max des pixels :", np.min(dataset_gris_train[0][1]), np.max(dataset_gris_train[0][1]))
        ...
        for fichier in os.listdir(chemin_sous_dossier):
            ...
            try:
                with Image.open(chemin_image) as img:
                    pixels = np.array([sum(pixel) // 3 for pixel in img.getdata()], dtype=np.float32)/255
                    pixels = pixels.reshape(-1, 1)
                    dataset_gris_test.append((int(label), pixels))
            except Exception as e:
                print(f" Erreur avec {chemin_image}: {e}")
print("Valeurs min/max des pixels :", np.min(dataset_gris_test[0][1]), np.max(dataset_gris_test[0][1]))
